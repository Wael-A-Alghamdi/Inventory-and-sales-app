...
public class PaymentMethodController implements Initializable {
  ...
  @Override
    public void initialize(URL url, ResourceBundle rb) {
         ObservableList<String> list = FXCollections.observableArrayList("Cash","Card");
         cbSelect.setItems(list);
...
private void loadUI(String ui) throws IOException{
    ...
    @FXML
    private void Select1(ActionEvent event) throws IOException {
        String s = cbSelect.getSelectionModel().getSelectedItem().toString();
        
        if(s.equals("Cash")){
            loadUI("PaymentMethodCash");
        }
        else if(s.equals("Card")){
            loadUI("PaymentMethodCard");
        }
